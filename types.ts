...
export enum OrderStatus {
  ...
  DELIVERY_CONFIRMATION_PENDING = 'Waiting Showroom Confirmation',
...
export interface Order {
  ...
  isCommissionPaid?: boolean; // NEW: Tracks if 5% commission is paid
...
export interface UserWallet {
  ...
  workWallet: number; // Income from direct labor
  bookingWallet: number; // NEW: Funds collected from Showroom (Admin Only)
  withdrawalWallet: number;
  pendingWithdrawal: number;
  performanceWallet: number;
  uplineIncome: number;
  downlineIncome: number;
  magicIncome: number; // NEW: Magic Income Wallet
...
export interface PaymentRequest {
  ...
  sourceWallet?: string; // NEW: Tracks which wallet the money is coming from
  utr?: string; 
  amount: number;
  mode: string;
